...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/v8/RestoreSessions/{restore_session_id}/organization/mailboxes/{mailbox_id}/folders/{folder_id}/exportToPst".format(restore_session_id=quote(str(restore_session_id), safe=""),mailbox_id=quote(str(mailbox_id), safe=""),folder_id=quote(str(folder_id), safe=""),),
    }
...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> ExchangeFolderExportToPstActionResponse200 | RESTExceptionInfo:
...
def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[ExchangeFolderExportToPstActionResponse200 | RESTExceptionInfo]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    r""" Export Data from Folders

     Exports backed-up data from an organization mailbox folder with the specified ID to a PST file. <div
    ...
    The request command will look for a specified keyword in item names and fields inside the
    organization mailbox folder. The backed-up data with a specified keyword then will be exported to a
    PST file and placed in a temporary folder on the Veeam Backup for Microsoft 365 server. After that,
    the PST file will be transferred as application/octet-stream media to the client. To download, read,
    convert to PST or perform other actions with the octet-stream, use features of programming
    languages.
